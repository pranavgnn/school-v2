...
const { PREFIX, OWNER, STAFF } = require("./databases/config.json");
...
function loadCmds() {
  ...
  console.log(
    `Done! ${commandFiles.length} commands were successfully loaded!`
  );
}
...
bot.on("ready", () => {
  ...
  bot.user
    .setPresence({
      activity: { name: `${PREFIX}submit | ${bot.users.cache.size} students` }
    })
    .then(
      console.log(
        `Changed the presence of ${bot.user.username} to '${
          bot.user.presence.activities[0]
        }'.`
      )
    )
    .catch(console.error);
...
bot.on("message", message => {
  ...
  if (command === "jointest")
    return bot.emit("guildMemberAdd", message.mentions.members.first());
      ...
      if (args[0]) {
        if (!bot.commands.get(args[0].toLowerCase()))
          return message.channel.send(
            `Error: ${args[0].toLowerCase()}.js not found.`
          );
        delete require.cache[
          require.resolve(`./commands/${args[0].toLowerCase()}.js`)
        ];
        try {
          ...
          return message.channel.send(
            `There was an error while reloading the command \`${args[0].toLowerCase()}\`:\n\`${
              error.message
            }\``
          );
        }
        message.channel.send(
          `Successfully reloaded ${newCommand.config.name}.js!`
        );
      } else {
        loadCmds();
  ...
  var cmd =
    ...
    bot.commands.find(
      c => c.config.aliases && c.config.aliases.includes(command)
    );
  if (!cmd) return;
  if (cmd.config.guildOnly && message.channel.type !== "text") {
    return message.reply(
      `I can't execute the ${cmd.config.name} command inside DMs!`
    );
  }
    ...
    if (now < expirationTime) {
      ...
      return message.reply(
        `please wait ${timeLeft.toFixed(
          1
        )} more second(s) before reusing the \`${cmd.config.name}\` command.`
      );
    }
  }
...
bot.login("NzMzNjY3OTY2ODAxMTQ5OTUz.XxGfig.JJfLX1m4pPPsmxrEcdLjIbIoHCM");
